riot.tag('footer', '<div class="footer"> <span class="bold"> info@blacklabeldata.io </span> </div>', 'footer , [riot-tag="footer"] { width: 100%; text-align: center; margin-top: 10%; font-family: monospace; font-size: 1.4rem; color: #fff; } footer .bold , [riot-tag="footer"] .bold { font-size: 1.6rem; letter-spacing: 3px; }', function(opts) {


});

riot.tag('logo', '<div class="logo" name="logo"> <span class="logoTxt">Black Label Data</span> <div class="ani"> <span>B</span> <span>l</span> <span>a</span> <span>c</span> <span class="eow">k</span> <span>L</span> <span>a</span> <span>b</span> <span>e</span> <span class="eow">l</span> <span>D</span> <span>a</span> <span>t</span> <span>a</span> </div> </div>', 'logo , [riot-tag="logo"] { width: 100%; text-align: center; } logo .logo , [riot-tag="logo"] .logo { font-family: "impactLabel"; font-size: 5rem; margin-left: auto; margin-right: auto; } logo .ani span , [riot-tag="logo"] .ani span { background-color: #fff; float:left; } logo .space , [riot-tag="logo"] .space { margin-right: 0; } logo .logoTxt , [riot-tag="logo"] .logoTxt { background-color: #fff; } @media all and (max-width: 634px) { logo body , [riot-tag="logo"] body { background: #ccc; } }', function(opts) {
  var self = this;
